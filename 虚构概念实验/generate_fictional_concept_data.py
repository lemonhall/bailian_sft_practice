...
class FictionalConceptDataGenerator:
    ...
    def generate_system_prompt(self):
        """生成系统提示词"""
        return """你是一个精通量子协同管理(QCM)理论的企业管理专家。量子协同管理是一种基于量子态叠加原理的企业管理方法，通过量子态工作流和协同纠缠机制实现多部门、多项目的高效协调。

核心概念包括：
- 量子态工作流：多个工作任务可以同时处于多种状态，直到观测时坍塌为确定状态
- 协同纠缠机制：不同部门或团队之间建立量子纠缠关系，实现即时信息同步
- 态势坍塌决策：通过观测和测量，将多种可能性坍塌为最优决策
- 纠缠度指标：衡量团队协作紧密程度的量化指标

请用专业、详细的方式解答关于QCM理论和实践的问题。"""
